...
/**
 * Cache of DBs so they aren't loaded repeatedl * @type {Object<string, Datastore>}
 */
const DB_CACHE = {};
...
function compile() {
  ...
  if ( !fs.existsSync(PACK_SRC) ) fs.mkdirSync(PACK_SRC, { recursive: true, mode: 0o775 });
  const folders = fs.readdirSync(PACK_SRC, { withFileTypes: true }).filter(file =>
    file.isDirectory() && ( !packName || (packName === file.name) )
  );
...
aram {object} data  Data for the entry being extracted.
 * @param {string} pack  Name of the pack.
